...
vector<vector<int>> vMultiply(vector<vector<long double>> vec1, vector<vector<int>> vec2) {
...
vector<vector<int>> consolidateVectors(vector<vector<int>> vectors, vector<vector<long double>> inverse) {
...
vector<vector<long double>> inverse(vector<vector<int>> adjoint, double determinant) {
	vector<vector<long double>> inverse(3, vector<long double>(3));

	for (int i = 0; i < 3; i++)
		for (int j = 0; j < 3; j++)
			inverse[i][j] = static_cast<long double>(adjoint[i][j]) / determinant;
...
int main() {
	...
	vector<vector<long double>> inverseKey = inverse(adjoint(cofactor), determinant);
